...
import torch
from huggingface_hub import HfApi, ModelCard, create_repo, login
...
def download_model(model_id):
    ...
    if os.path.exists(model_name):
        ...
        result = subprocess.run(
            ["git", "clone", f"https://huggingface.co/{model_id}"],
            check=True,
            capture_output=True,
            text=True,
        )
...
def quantize_gguf(model_name, quantization_format="q4_k_m"):
    ...
    os.system(
        f"python llama.cpp/convert_hf_to_gguf.py {model_name} --outtype f16 --outfile {fp16}"
    )
...
def quantize_awq(model_name, bits=4, group_size=128, version="GEMM", zero_point=True):
    ...
    model = AutoAWQForCausalLM.from_pretrained(
        model_path, safetensors=True, low_cpu_mem_usage=True, local_files_only=True
    )
    tokenizer = AutoTokenizer.from_pretrained(
...
def upload_model(username, model_name, quantization_method, save_folder, hf_token):
    ...
    try:
        if quantization_method == "GGUF":
            ...
            api.upload_folder(folder_path=save_folder, repo_id=repo_id, token=hf_token)
...
def main():
    parser = argparse.ArgumentParser(
        description="EasyQuant: Simplified Quantization Tool"
    )
    ...
    if not torch.cuda.is_available():
        print(
            "Warning: CUDA is not available. This may significantly slow down the quantization process."
        )
        print(
            "If you have a CUDA-capable GPU, please ensure you have the correct drivers and CUDA toolkit installed."
        )
    else:
    ...
    upload_model(
        args.username, model_name, args.method.upper(), save_folder, args.hf_token
    )
    print(
        f"Quantized model uploaded to: {args.username}/{model_name}-{args.method.upper()}"
    )
